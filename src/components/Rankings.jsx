...
export default function Rankings({ players, matches, onSelectPlayer }) {
  ...
  if (sorted.length === 0) {
    return <div className="empty-state">No players yet. Add some friends!</div>;
  }
          ...
          {sorted.map((p, i) => {
            ...
            return (
              <tr key={p.id} className={isTop ? 'row-top' : ''}>
                ...
                {/* Elo */}
                <td className="td-elo" style={{ color: isTop ? '#FFD700' : '#f0f0f0' }}>
                  {p.elo}
                </td>
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{
                      flex: 1, height: 4, borderRadius: 2, maxWidth: 60,
